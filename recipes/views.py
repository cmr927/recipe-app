...
from recipe_ingredients.models import (
    RecipeIngredient,
...
class RecipeListView(LoginRequiredMixin, ListView):  # class-based “protected” view
    ...
    def post(self, request):
        ...
        ingredient_qs = RecipeIngredient.objects.filter(
            recipe__name__icontains=recipe_title,
            ingredient__name__icontains=ingredient_title,
        ).distinct()
        if qs and ingredient_qs:  # if data found
            # convert the queryset values to pandas dataframe
            recipes_df = pd.DataFrame(qs.values())
            ingredients_df = pd.DataFrame(ingredient_qs.values())
            ingredients_df["ingredient_name"] = ingredients_df["ingredient_id"].apply(
    ...
    if request.method == "POST":
        ...
        for i in ingredients:
            ...
            RecipeIngredient.objects.create(
                ingredient=ingredient, recipe=r, quantity="1oz"
            )
            r.ingredients.add(i)
        r.calc_difficulty()
        r.save()
